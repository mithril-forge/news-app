...
async def generate_and_connect_news(delta: timedelta) -> None:
    ...
    async with get_session_context() as session:
        ...
        input_news = await input_news_service.get_input_news_by_delta(delta=delta, has_parsed_news=False)
        input_news_ids = [inp.id for inp in input_news]
        new_news_groups = await article_generation_service.choose_input_news_for_new_articles(
            input_news_ids=input_news_ids
        )
        ...
        for group in new_news_groups:
            ...
            await article_generation_service.create_new_article_from_input_news(input_news_ids=group.input_news_ids,
                                                                                importancy=group.importancy)
