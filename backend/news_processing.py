...
async def get_input_news_and_parse(
    adjust_parse_date: bool = True, delta: timedelta = timedelta(days=1)
) -> list[int]:
    """Official scenario that will be used to parse new input news and generates the parsed ones from them by CLI"""
    logger.info(
        f"Starting CLI input news parsing with adjust_parse_date: {adjust_parse_date}, delta: {delta}"
    )
    ...
    async with get_session_context() as session:
        ...
        result = await input_news_service.scrap_and_save_input_news(
            delta=delta, adjust_parse_date=adjust_parse_date
        )
...
async def generate_and_connect_news(delta: timedelta):
    ...
    async with get_session_context() as session:
        ...
        input_news = await input_news_service.get_input_news_by_delta(
            delta=delta, has_parsed_news=False
        )
        ...
        logger.info(
            f"Parsed existing articles with ids picked for enrichment: {[connected_news_ids]}"
        )
        for news_id in connected_news_ids:
            logger.info(f"Enriching article {news_id}")
            await article_generation_service.enrich_existing_article(
                parsed_news_id=news_id
            )
        await session.flush()
        ...
        for group in new_news_groups:
            ...
            await article_generation_service.create_new_article_from_input_news(
                input_news_ids=group
            )


async def generate_picture_for_news(
    news_id: int, commit_transaction: bool = False
) -> None:
    # TODO: Rework to support latest news without picture instead of passing ID
    """Generates picture for the news with specific ID"""
    logger.info(
        f"Generating picture for news ID: {news_id}, commit: {commit_transaction}"
    )
    ...
    async with get_session_context(commit_transaction=commit_transaction) as session:
        ...
        await article_generation_service.generate_and_attach_image_to_news(
            news_id=news_id
        )
...
def main():
    ...
    parse_parser = subparsers.add_parser(
        "parse", help="Parse input news and generate new articles"
    )
    parse_parser.add_argument(
        "--days", type=int, default=1, help="Number of days to look back for news"
    )
    parse_parser.add_argument(
        "--adjust-date",
        action="store_true",
        help="Adjust parse date based on latest timestamp",
    )
    # Generate news
    ...
    picture_parser = subparsers.add_parser(
        "generate-picture", help="Generate picture for a specific news article"
    )
    picture_parser.add_argument("news_id", type=int, help="ID of the news article")
    picture_parser.add_argument(
        "--commit", action="store_true", help="Commit transaction to database"
    )
    ...
    try:
        if args.command == "parse":
            ...
            logger.info(
                f"Running parse command with {args.days} days, adjust_date: {args.adjust_date}"
            )
            loop.run_until_complete(
                get_input_news_and_parse(
                    adjust_parse_date=args.adjust_date, delta=delta
                )
            )
            ...
            loop.run_until_complete(
                generate_picture_for_news(
                    news_id=args.news_id, commit_transaction=args.commit
                )
            )
