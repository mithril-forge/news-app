...
class InputNewsService:
    ...
    async def add_or_update_input_news_batch(
        ...
        for model in orm_models:
            ...
            if existing:
                logger.debug(
                    f"Updating existing input news with source_url: {model.source_url}"
                )
    ...
    async def scrap_and_save_input_news(
        self, adjust_parse_date: bool, delta: timedelta
    ) -> list[InputNewsWithID]:
        """
    ...
    async def clear_old_input_news(self, delta: timedelta) -> None:
        ...
        old_input_news = await self.input_news_repo.get_by_time_delta(
            delta=delta, newer=False, has_parsed_news=False
        )
        ...
        logger.info(
            f"Archived {len(old_input_news)} old input news items to: {archive_path}"
        )
    ...
    async def scrap_input_news(
        ...
        """
        logger.info(
            f"Scraping input news with delta: {delta}, max_articles_per_site: {max_articles_per_site}"
        )
        if websites is not None:
            ...
            crawl_result = crawl_czech_news(
                time_delta=delta, max_articles_per_site=max_articles_per_site
            )
        ...
        logger.debug(
            f"Retrieved {len(articles)} total articles from {len(articles_by_domain)} domains"
        )
        ...
        for article in articles:
            ...
            publish_date = (
                article.publish_date if article.publish_date else datetime.now()
            )
        ...
        logger.info(
            f"Scraped {len(input_news_list)} articles from {len(articles_by_domain)} domains"
        )
    ...
    async def get_input_news_by_delta(
        ...
        """
        logger.debug(
            f"Getting input news by delta: {delta}, has_parsed_news: {has_parsed_news}"
        )
        result = await self.input_news_repo.get_by_time_delta(
            delta=delta, has_parsed_news=has_parsed_news
        )
    ...
    async def get_input_news_by_ids_lite(
        ...
        """
        logger.debug(
            f"Getting input news for input news ids: {input_news_ids}, has_parsed_news: {has_parsed_news}"
        )
    ...
    async def get_parsed_with_input_news(
        ...
        logger.info(
            f"Retrieved {len(converted_result)} parsed news with input by delta"
        )
        return converted_result

    async def connect_input_with_parsed(
        self, parsed_id: int, input_id: int
    ) -> InputNewsWithID:
        """
        Links a parsed news entry with its original input news entry.
        """
        logger.info(
            f"Connecting input news ID {input_id} with parsed news ID {parsed_id}"
        )
        db_result = await self.input_news_repo.update_parsed_news_id(
            input_id=input_id, parsed_news_id=parsed_id
        )
        result = input_news_to_schema(db_result)
        logger.info(
            f"Successfully connected input news ID {input_id} with parsed news ID {parsed_id}"
        )
        return result
